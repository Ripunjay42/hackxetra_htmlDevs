...
const PostFeed = ({ userId }) => {
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      const hasLiked = currentPost.Like?.some(like => like.userId === parseInt(userId));
      ...
      setPosts(prevPosts => prevPosts.map(post => {
        if (post.id === postId) {
          if (hasLiked) {
            return {
              ...post,
              Like: post.Like.filter(like => like.userId !== parseInt(userId))
            };
          } else {
            return {
              ...post,
      ...
      setPosts(prevPosts => prevPosts.map(post => {
        if (post.id === postId) {
          const hasLiked = post.Like?.some(like => like.userId === parseInt(userId));
          if (hasLiked) {
            return {
              ...post,
  ...
  const handleComment = async (postId) => {
    try {
      ...
      const response = await axios.post(`http://localhost:3001/api/posts/${postId}/comment`, {
        userId,
